...
import { FaGithub, FaYoutube, FaLinkedin, FaKaggle,  } from "react-icons/fa6";
...
export default async function Footer() {
  ...
  return (
    <Bounded as="footer" className="text-slate-600">
      <div className="container mx-auto mt-20 flex flex-col items-center justify-between gap-0 py-0 sm:flex-row ">
        ...
        {isFilled.link(settings.data.linkedin_link) && (
          ...
          {isFilled.link(settings.data.kaggle_link) && (
            <PrismicNextLink
              ...
              className="p-2 text-2xl text-slate-300 transition-all duration-150 hover:scale-125 hover:text-yellow-400"
              aria-label={settings.data.name + " on Kaggle"}
            >
              <FaKaggle />
            </PrismicNextLink>
          )}
